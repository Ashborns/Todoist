// * Code By Nazand Code
// * Fitur MangaDex (Dibuat Krn Gabut)
// * Banyak Error? soalnya buat codenya buru"v:
...
async function searchManga(title) {
    try {
        ...
        if (!mangaList.length) throw new Error('Manga tidak ditemukan.');
...
async function getMangaChapters(mangaId) {
    try {
        ...
        if (!chapters.length) throw new Error('Tidak ada chapter tersedia.');
...
const handler = async (m, { conn, args }) => {
    if (args.length < 2) {
        return m.reply('Tolong masukkan judul manga dan chapter yang ingin dicari (contoh: .mangad One Piece 10).');
    }
    ...
    try {
        ...
        m.reply(`Manga ditemukan: ${manga.attributes.title.en}`);
        
        const chapters = await getMangaChapters(mangaId);
        const chapterData = chapters.find(ch => ch.attributes.chapter === chapterRequested);
        
        if (!chapterData) {
            return m.reply(`Chapter ${chapterRequested} tidak ditemukan untuk manga ${mangaTitle}.`);
        }
        
        const imageUrls = await getChapterImages(chapterData.id);
        const outputPath = path.join('/tmp', `${manga.attributes.title.en} - Chapter ${chapterRequested}.pdf`);
